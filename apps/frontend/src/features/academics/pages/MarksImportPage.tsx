...
import { MarksUploadStep } from '../components/import/MarksUploadStep';
import { MarksPreviewStep } from '../components/import/MarksPreviewStep';
import { MarksConfigureStep } from '../components/import/MarksConfigureStep';
import { MarksProgressStep } from '../components/import/MarksProgressStep';
import { MarksCompleteStep } from '../components/import/MarksCompleteStep';
import {
  marksImportService,
...
const STEPS: StepDefinition[] = [
  { id: 1, label: 'Upload', hint: 'Select assessment & upload Excel', icon: UploadCloud },
  { id: 2, label: 'Preview', hint: 'Review breakdown & student matches', icon: ListChecks },
...
export function MarksImportPage() {
  ...
  const { data: preview, isLoading: loadingPreview } = useQuery({
    queryKey: ['admin', 'marks-imports', batchId, 'preview'],
    queryFn: () => marksImportService.getPreview(batchId as string),
    enabled: Boolean(batchId) && step >= 2 && step <= 3,
  });

  const { data: history, isLoading: loadingHistory } = useQuery({
    queryKey: ['admin', 'marks-imports', 'history'],
    queryFn: () => marksImportService.getHistory(15),
  });
  ...
  const handleStart = async (config: MarksImportConfiguration) => {
    if (!batchId) return;
    setIsStarting(true);
    setStartError(null);
    try {
      await marksImportService.execute(batchId, config);
      setStep(4);
    } catch (err: any) {
      setStartError(err?.response?.data?.error?.message ?? 'The marks import could not be started.');
    } finally {
      setIsStarting(false);
    }
  };
  ...
  return (
    <>
      ...
      />
      <PageHeader
        title="Enterprise Marks Import"
        subtitle="Bulk import student marks for any assessment structure (Mid Written, Open Book, Objective, Seminar, Lab, Viva) with question-wise component breakdowns."
      />
      ...
      {runError && (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-rose-500/20 bg-rose-500/10 p-4">
          ...
          <div className="min-w-0 flex-1 text-xs leading-relaxed text-rose-700 dark:text-rose-300">
            <p className="font-bold">The marks import failed and was completely rolled back.</p>
      ...
      {step === 1 && (
        <div className="space-y-6">
          ...
          />
          <MarksDashboard history={history} isLoading={loadingHistory} />
        </div>
      )}
      ...
      {step === 5 && batchId && (
        <MarksCompleteStep batchId={batchId} onStartAnother={reset} />
      )}
    </>
  );
}
...
function MarksDashboard({
  ...
  history?: Awaited<ReturnType<typeof marksImportService.getHistory>>;
  isLoading: boolean;
}) {
  if (isLoading) {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            Marks Import History
          </CardTitle>
          <CardDescription>
            History of student marks uploads and assessment structures.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!history || history.items.length === 0 ? (
            <EmptyState
              icon={UploadCloud}
              title="No marks imports yet"
              description="Upload your first marks spreadsheet above to import assessment marks."
            />
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {history.items.map((item) => (
                  <TableRow key={item.batch_id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{item.assessment_code}</Badge>
                    </TableCell>
