...
interface AttendanceCompleteStepProps {
  batchId: string;
  onStartAnother: () => void;
}

export function AttendanceCompleteStep({ batchId, onStartAnother }: AttendanceCompleteStepProps) {
  const { data: summary, isLoading } = useQuery({
    queryKey: ['admin', 'attendance-imports', batchId, 'summary'],
    queryFn: () => attendanceImportService.getSummary(batchId),
  });
    ...
    <div className="space-y-6">
      ...
      <Card className="border-emerald-500/30 bg-emerald-500/5">
        <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">Attendance Import Completed!</h2>
              <p className="text-xs text-muted-foreground">
                Attendance Date: {summary.attendance_date} | {summary.records_created} created, {summary.records_updated} updated.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => attendanceImportService.downloadReport(batchId)}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Outcome for each student roll number in the uploaded file.</CardDescription>
        </CardHeader>
        <CardContent>
